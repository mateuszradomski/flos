...
contract IndexOf {
        ...
        returns (int) // _a = string to search, _b = string we want to find
    {
        ...
        (2 ** 128 - 1)) // since we have to be able to return -1 (if the char isn't found or input error), this function must return an "int" type with a max length of (2^128 - 1)
        {
            whatwastheval = -1;
            return -1;
        } else {
            ...
            for(uint i = 0; i < a.length; i++) {
                if(a[i] == b[0]) // found the first char of b
                {
                    ...
                    a[i +
                    subindex] ==
                    b[subindex]) // search until the chars don't match or until we reach the end of a or b
                    {
                        subindex++;
                    }
                    if(subindex == b.length) {
