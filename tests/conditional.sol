...
contract Conditional {
    function foo() {
        ...
        asset == ETH
            ? require(address(uint160(msg.sender)).send(quantity), "failed to transfer ether") // explicit casting to `address payable`
            : transferTokensToAccountSecurely(Token(asset), quantity, msg.sender);
        asset == ETH ? true : transferTokensToAccountSecurely(Token(asset), quantity, msg.sender);
        asset == ETH ? require(address(uint160(msg.sender)).send(quantity), "failed to transfer ether") : false;
    }
